...
async function seedModules() {
  ...
  const modules = [
    ...
    { id: '4724a72f-85f1-44e4-a278-1838ee44d2e4', course_id: A1_COURSE_ID, title: 'Números 1-10', description: 'Los números básicos', order_num: 5 },
    ...
    { id: '1fbfe140-e1bc-4b08-8407-6a150ffdca8b', course_id: A2_COURSE_ID, title: 'Adverbs of Frequency', description: 'Adverbs of frequency', order_num: 8 },
  ...
  console.log(`✅ ${modules.length} modules seeded successfully!`)
  return true
}
  ...
  const lessons = [
    {
      ...
      content: 'Aprende saludos básicos en inglés: Hello, Good morning, etc.',
      order_num: 1,
      lesson_type: 'theory'
    },
    {
      ...
      module_id: '7f851801-3f51-4198-a141-2288ca60f184',
      title: 'Quiz: Saludos',
      content: 'Pon a prueba tus conocimientos',
      order_num: 2,
      lesson_type: 'quiz'
    }
  ]

  const { error } = await supabase.from('lessons').upsert(lessons, { onConflict: 'id' })
...
async function seedExercises() {
  ...
  if (quizError || !quizLesson) {
    console.error('Quiz lesson not found:', quizError?.message)
    return false
  }
  ...
  const exercises = [
    {
      ...
      options: JSON.stringify(['What\'s up?', 'Hey!', 'Good morning', 'Hi!']),
    ...
    },
    {
      ...
      options: JSON.stringify(['Good morning', 'Good evening', 'Good afternoon', 'Good night']),
      exercise_type: 'multiple_choice',
      points: 25
    }
  ]
