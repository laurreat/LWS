...
import { Volume2, Home, Trophy, CheckCircle, XCircle, Ear, Mic, ChevronRight } from "lucide-react";
...
export default function ListeningPage() {
  ...
  const finishGame = useCallback(() => {
    if (selectedLevel && !hasFinishedTriggered) {
      ...
      playGame("listening", score, score, selectedLevel);
        ...
        >
          <div className="text-center mb-12">
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-pink-500/10 rounded-full mb-4">
              <Ear className="w-5 h-5 text-pink-500" />
              <span className="text-sm font-medium text-pink-600">Escuchar</span>
            </div>
          ...
          <div className="space-y-4">
            {([
              { level: "A1" as GameLevel, icon: Ear, label: "Principiante", desc: "Frases básicas", color: "from-green-500 to-emerald-500" },
              { level: "A2" as GameLevel, icon: Mic, label: "Elemental", desc: "Frases intermedias", color: "from-blue-500 to-cyan-500" },
              { level: "B1" as GameLevel, icon: Volume2, label: "Intermedio", desc: "Frases avanzadas", color: "from-purple-500 to-violet-500" },
            ]).map((item, index) => (
              <motion.div
                key={item.level}
              ...
              >
                <Card 
                  hover 
                  className={`cursor-pointer border-2 border-transparent hover:border-pink-500/20 transition-all duration-300`}
